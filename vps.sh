...
check_installed() {
    if command -v "$1" &> /dev/null || [ -f "/usr/bin/$1" ] || [ -f "/usr/local/bin/$1" ] || [ -d "/www/server/panel" -a "$1" = "bt" ]; then
        ...
        echo -e "${YELLOW}你可以直接使用快捷命令: ${RED}$3${NC}"
...
show_menu() {
    ...
    echo -e "${YELLOW} 5.${NC} 安装 Debian 11 系统"
    echo -e "${YELLOW} 6.${NC} 安装 Win10 LTSC 系统 (秋水逸冰)"
    echo -e "${YELLOW} 7.${NC} 安装 Win10 系统 (veip007版)"
    echo -e "${YELLOW} 8.${NC} 安装 aaPanel 面板 (AaronYES开心版)"
    echo -e "${YELLOW} 9.${NC} 安装 aaPanel 面板 (官方正式版)"
    echo -e "${YELLOW} 10.${NC} 安装 Docker 运行环境"
    echo -e "${YELLOW} 11.${NC} 安装 Realm 端口转发工具"
    echo -e "${YELLOW} 12.${NC} 安装 ServerStatus 监控探针"
    echo -e "${YELLOW} 13.${NC} 安装 Xray 代理服务 (233boy版)"
    echo -e "${YELLOW} 14.${NC} 安装 sing-box 代理服务 (233boy版)"
    echo -e "${YELLOW} 15.${NC} 安装 XrayR 后端对接 (官方正式版)"
    echo -e "${YELLOW} 16.${NC} 安装 XrayR 后端对接 (柚子版脚本)"
    echo -e "${BLUE}--------------------------------------------------${NC}"
    echo -e "${RED} 0.${NC} 退出脚本（或者双击回车）${NC}"
    echo -e "${BLUE}==================================================${NC}"
}
...
while true; do
    ...
    case $choice in
        ...
        6)
            ...
            ;;
        7)
            read -p "设置 Win10 登录密码 (回车默认: $DEFAULT_PASS): " v_pass
            v_pass=${v_pass:-$DEFAULT_PASS}
            read -p "设置 RDP 远程登录端口 (回车默认: 3389): " v_port
            v_port=${v_port:-3389}
            echo -e "${RED}确认信息：Win10 | 用户名 Administrator | 密码 $v_pass | RDP端口 $v_port${NC}"
            echo -e "${YELLOW}脚本将在 10 秒后执行安装...${NC}"
            sleep 10
        ...
        8 | 9)
            ...
            ;;
        10) check_installed "docker" "Docker" "docker ps" || { read -p "按回车继续..."; continue; }; check_docker; read -p "按回车继续..." ;;
        11) check_installed "realm" "Realm" "./realm.sh" || { read -p "按回车继续..."; continue; }; wget https://raw.githubusercontent.com/jinqians/realm/refs/heads/main/realm.sh && chmod +x realm.sh && ./realm.sh ;;
        12)
        ...
        13) check_installed "xray" "Xray" "xray" || { read -p "按回车继续..."; continue; }; bash <(wget -qO- -o- https://github.com/233boy/Xray/raw/main/install.sh) ;;
        14) check_installed "sb" "sing-box" "sb" || { read -p "按回车继续..."; continue; }; bash <(wget -qO- -o- https://github.com/233boy/sing-box/raw/main/install.sh) ;;
        15) check_installed "XrayR" "XrayR 官方" "XrayR" || { read -p "按回车继续..."; continue; }; bash <(curl -Ls https://raw.githubusercontent.com/XrayR-project/XrayR-release/master/install.sh) ;;
        16) check_installed "xrayr" "XrayR 柚子" "xrayr" || { read -p "按回车继续..."; continue; }; wget -N https://raw.githubusercontent.com/youzi3/XrayR-script/main/install.sh && bash install.sh ;;
        0) exit 0 ;;
